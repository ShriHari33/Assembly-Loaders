...
int main()
{
    ...
    final_file.seekg(0, std::ios::beg);
    std::ofstream output_file("bytecode.txt");
    if(!output_file.is_open())
    {
        std::cerr << "Error opening bytecode file" << std::endl;
        return 1;
    }
    ...
    while(std::getline(final_file, line))
    {
        if(line[0] == 'H')
        {
            ...
            output_file << ". " << name_of_prog << " begins at address: " << std::hex << "0x" << start_addr << "\n\n";
        }
        else if(line[0] == 'T')
        {
